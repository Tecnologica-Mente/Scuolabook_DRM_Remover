...
auth = input('Enter Cookie:\n')
effective_pages = input('Enter the effective number of pages (leave blank for self-detection, but may not be accurate and in the PDF of the book may be missing the final pages):\n')
HEADERS = {'Cookie':auth}
...
def book_info():
	...
	if(effective_pages.isnumeric()):
		book_metadata[0]['ws_num_pages'] = effective_pages
	...
	#return Book(book_metadata[0]['ws_title'], book_metadata[0]['ws_author'], book_metadata[0]['ws_isbn'], effective_pages, toc, book_metadata[0]['ws_book_id'])

# Not used. Replaced with the standard one
...
def downloadbook():
	...
	bar = progressbar.ProgressBar(page_number)
	pages = []
	if page_number > 500: # 414 Request-URI Too Large
		...
		attachment = [f'pages[]={n}&' for n in range(1, int(book.pages)+1)]  # Fixes the issue of all downloaded pages
		pages_req = requests.get(f'https://webapp.scuolabook.it/books/{book.identifier}/pages?{"".join(attachment)}', headers=HEADERS).json()['pages'].values()
		pages.extend(list(pages_req))
	bar.start()
	time.sleep(0.1)
	...
	for n,page in enumerate(pages):
		...
		time.sleep(0.1)
		if(n+1<page_number-2):   # To compensate for the delay between processing time and writing to the screen. Put at the end of the cycle did not work
			bar.update(n+1)  # Why pages start from 1 and not 0
		else:
...
downloadbook()
print()
print(f"Book downloaded successfully! You will find it in the script folder.")